// shared visual for both /dashboard (real user data) and /demo/dashboard
// (mock data). pure presentation - it takes recents/presets in and
// renders the same surfaces regardless of source.
...
type Props = {
  ...
  recents: RecentConcept[];
  presets: AnalogyPreset[];
  analyticsEnabled: boolean;
  // when omitted, the panel is read-only (used by the demo).
  analyticsControl?: React.ReactNode;
  // links route differently in demo mode.
  links: {
    app: string;
    history: string;
    topics: string;
    account: string;
    recentHref: (id: string) => string;
  };
...
export const DashboardView = ({
  ...
  const analytics = useMemo(() => {
    ...
    const topSystems = Array.from(systemCounts.entries()).sort((a, b) => b[1] - a[1]).slice(0, 5);
  ...
  return (
    <section className="container max-w-4xl py-12">
      ...
      <h1 className="font-serif-display text-4xl tracking-tight">{greeting}</h1>
      {subtitle && <p className="mt-3 max-w-2xl text-sm text-foreground/70">{subtitle}</p>}
      ...
      <div className="mt-10 grid gap-4 sm:grid-cols-2">
        <Tile to={links.app} title="open the app" sub="translate a new concept" />
        <Tile to={links.history} title="history" sub="browse and tag past translations" />
        <Tile to={links.topics} title="topics" sub="rename, merge, or clear tag clusters" />
        <Tile to={links.account} title="account" sub="sync, export, sign out" />
      </div>
      ...
      <section className="mt-12">
        <div className="mb-4 flex items-center justify-between gap-3">
          ...
          <Link to={links.history} className="text-xs text-muted-foreground hover:text-foreground">
            see all →
          </Link>
        </div>
        {recents.length === 0 ? (
          <p className="surface-paper p-6 text-sm text-muted-foreground">nothing yet.</p>
        ) : (
          <ul className="divide-y divide-border/60 surface-paper">
            {recents.slice(0, 6).map((r) => {
              ...
              return (
                <li key={r.id}>
                  <Link to={links.recentHref(r.id)} className="flex items-center justify-between gap-3 p-4 hover:bg-secondary/40">
...
const Tile = ({ to, title, sub }: { to: string; title: string; sub: string }) => (
  <Link to={to} className="surface-paper p-5 hover:bg-secondary/40">
    <p className="font-serif-display text-lg">{title}</p>
